...
Важно помнить, что по тем, либо иным причинам могут отсутствовать те или иные выражения в записи цикла for и он всё еще убедт функционален. (примеры придумайте сами или найдите в гугл)
...
В дальнейшем мы узнаем ещё больше об этом типе цикла.

### Циклы while и do-while

Много о них говорить не будем(они очень понятны после for), просто рассмотрим примеры синтаксиса. За доп пояснениями дорогу мы помним)
...
int main()
{
    ...
    while (i < 1000) // пока i меньше 1000
    {
        i++;
        sum += i;
    }
...
Отличается лишь тем, что в do while первая итерация выполняется всегда, а в while только при выполнении условия. Для разных ситуаций мы сами подбираем необходимый нам вид цикла.
...
Пример для switch мы уже встречале выше.
...
for (i = 0; i < 5; i++) // цикл по i
  {
    if (i == 3)
      break; // выход из цикла for на 4-ом шаге

    //проверка того что этот оператор выполняется
    cout << "i = " << i << endl;
  }
```

Также применимо к обоим видам while.
...
for (i = 0; i < 5; i++) // цикл по i
  {
    if (i == 3)
      continue; // переход к следующей итерации, где i == 4
...
Используется для пропуска того, либо иного нежелательного выполнения определнной в if итерации
